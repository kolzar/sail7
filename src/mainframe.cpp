...
// first line
	...
	pSail7->m_paij          = m_aij;
	pSail7->m_paijRef       = m_aijRef;
	pSail7->m_pRHS          = m_RHS;
	pSail7->m_pRHSRef       = m_RHSRef;
	QSail7::s_pNode         = m_Node;
	QSail7::s_pMemNode      = m_MemNode;
	QSail7::s_pWakeNode     = m_WakeNode;
	QSail7::s_pRefWakeNode  = m_RefWakeNode;
	QSail7::s_pPanel        = m_Panel;
	QSail7::s_pMemPanel     = m_MemPanel;
	QSail7::s_pWakePanel    = m_WakePanel;
	QSail7::s_pRefWakePanel = m_RefWakePanel;
...
void MainFrame::CreateActions()
{
	...
	connect(resetGraphLegend, SIGNAL(triggered()), pSail7, SLOT(OnResetGraphLegend()));
	...
	connect(twoGraphs, SIGNAL(triggered()), pSail7, SLOT(OnTwoGraphs()));
	...
	connect(allBoatPolarGraphsScalesAct, SIGNAL(triggered()), pSail7, SLOT(OnAllBoatPolarGraphScales()));
	...
	connect(allBoatPolarGraphsSettings, SIGNAL(triggered()), pSail7, SLOT(OnAllBoatPolarGraphSettings()));
...
void MainFrame::CreateDockWindows()
{
	...
	m_pSail7 = new QSail7;
	QSail7 *pSail7= (QSail7*)m_pSail7;
	m_pctrlSail7Widget->setWidget(pSail7);
	...
	QSail7::s_p2DWidget = m_p2DWidget;
	QSail7::s_p3DWidget = m_p3DWidget;
	pSail7->m_ArcBall.m_p3dWidget  = m_p3DWidget;
	pSail7->m_poaBoat      = &m_oaBoat;
	pSail7->m_poaBoatPolar = &m_oaBoatPolar;
	pSail7->m_poaBoatOpp   = &m_oaBoatOpp;
...
void MainFrame::CreateSail7Actions()
{
	...
	Sail7NewBoatAct = new QAction(tr("New Boat"), pSail7);
	Sail7NewBoatAct->setShortcut(QKeySequence(Qt::Key_F3));
	connect(Sail7NewBoatAct, SIGNAL(triggered()), pSail7, SLOT(OnNewBoat()));

	Sail7EditBoatAct = new QAction(tr("Edit Boat"), pSail7);
	Sail7EditBoatAct->setShortcut(QKeySequence(Qt::SHIFT + Qt::Key_F3));
	connect(Sail7EditBoatAct, SIGNAL(triggered()), pSail7, SLOT(OnEditCurBoat()));
	...
	connect(BoatPolarAct, SIGNAL(triggered()), pSail7, SLOT(OnSailPolarView()));
	...
	connect(Boat3DAct, SIGNAL(triggered()), pSail7, SLOT(OnSail3DView()));
	...
	connect(Sail73DPrefsAct, SIGNAL(triggered()), pSail7, SLOT(On3DPrefs()));
...
//	Sail73DScalesAct->setCheckable(true);
	connect(Sail73DScalesAct, SIGNAL(triggered()), pSail7, SLOT(OnGL3DScale()));
	...
	connect(Sail73DLightAct, SIGNAL(triggered()), pSail7, SLOT(OnLight()));
...
//	defineWPolar->setShortcut(tr("F6"));
	connect(defineBoatPolar, SIGNAL(triggered()), pSail7, SLOT(OnDefineBoatPolar()));
	...
	connect(deleteAllBoatOpps, SIGNAL(triggered()), pSail7, SLOT(OnDeleteAllBoatOpps()));	
	...
	connect(deleteAllBoatPolarOpps, SIGNAL(triggered()), pSail7, SLOT(OnDeleteAllBoatPolarOpps()));
	...
	connect(deleteCurBoatOpp, SIGNAL(triggered()), pSail7, SLOT(OnDeleteCurBoatOpp()));
	...
	connect(showBoatOppProperties, SIGNAL(triggered()), pSail7, SLOT(OnBoatOppProps()));
	...
	connect(showBoatPolarProperties, SIGNAL(triggered()), pSail7, SLOT(OnBoatPolarProps()));
	...
	connect(hideCurBoatPlrs, SIGNAL(triggered()), pSail7, SLOT(OnHideCurBoatPolars()));
	...
	connect(showCurBoatPlrs, SIGNAL(triggered()), pSail7, SLOT(OnShowCurBoatPolars()));
	...
	connect(deleteCurBoatPlrs, SIGNAL(triggered()), pSail7, SLOT(OnDeleteCurBoatPolars()));
	...
	connect(hideAllBoatPlrs, SIGNAL(triggered()), pSail7, SLOT(OnHideAllBoatPolars()));
	...
	connect(showAllBoatPlrs, SIGNAL(triggered()), pSail7, SLOT(OnShowAllBoatPolars()));
...
void MainFrame::DeleteBoat(void *pObj, bool bResultsOnly)
{

	if(!pObj) return;
	CBoat *pThisBoat = (CBoat*)pObj;
	SetSaveState(false);
	int i;
	...
	{
		pBoatOpp = (BoatOpp*)m_oaBoatOpp.at(i);
		if(pBoatOpp->m_BoatName == pThisBoat->m_BoatName)
		{
			m_oaBoatOpp.removeAt(i);
			if(pBoatOpp == pSail7->m_pCurBoatOpp) pSail7->m_pCurBoatOpp = NULL;
			delete pBoatOpp;
		}
		pSail7->m_pCurBoatOpp = NULL;
	}
	...
	{
		pBoatPolar = (BoatPolar*)m_oaBoatPolar.at(i);
		if (pBoatPolar->m_BoatName == pThisBoat->m_BoatName)
		{
			if(!bResultsOnly)
			{
				m_oaBoatPolar.removeAt(i);
				if(pBoatPolar == pSail7->m_pCurBoatPolar)
				{
					pSail7->m_pCurBoatPolar = NULL;
				}
				delete pBoatPolar;
			}
			else
	...
	{
		pBoat = (CBoat*)m_oaBoat.at(i);
		if (pBoat == pThisBoat)
		{
			m_oaBoat.removeAt(i);
			delete pBoat;
			if(pBoat == pSail7->m_pCurBoat)	pSail7->m_pCurBoat = NULL;
			break;
		}
	}
}
...
void MainFrame::DeleteProject()
{
	...
	for (i=m_oaBoatPolar.size()-1; i>=0; i--)
	{
		pObj = m_oaBoatPolar.at(i);
		m_oaBoatPolar.removeAt(i);
		delete (BoatPolar*)pObj;
	}
	for (i=m_oaBoatOpp.size()-1; i>=0; i--)
	{
		pObj = m_oaBoatOpp.at(i);
		m_oaBoatOpp.removeAt(i);
		delete (BoatOpp*)pObj; //TODO : cast
	}



	QSail7 *pSail7 = (QSail7*)m_pSail7;
	pSail7->m_pCurBoat      = NULL;
	pSail7->m_pCurBoatPolar = NULL;
	pSail7->m_pCurBoatOpp   = NULL;
	UpdateBoats();
	if(pSail7->m_iView==SAILPOLARVIEW)    pSail7->CreateBoatPolarCurves();
...
void MainFrame::OnSaveViewToImageFile()
{
	...
	if(pSail7->m_iView==SAIL3DVIEW)
	{
		pSail7->UpdateView();
		pSail7->SnapClient(FileName);
		return;
	}
	else pSail7->PaintView(painter);
...
void MainFrame::OnSelChangeBoatPolar(int i)
{
	...
	pSail7->SetBoatPolar(NULL, strong);

	UpdateBoatOpps();
	pSail7->SetBoatOpp(true);
	pSail7->SetControls();
	pSail7->UpdateView();
}
...
void MainFrame::OnStyle()
{
	...
	if(m_DisplaySettingsDlg.exec() ==QDialog::Accepted)
	{
		...
		pSail7->m_bResetglSailGeom = true;
...
void MainFrame::SetCentralWidget()
{
	...
	else if(pSail7->m_iView==SAIL3DVIEW)
	{
...
bool MainFrame::SerializeProject(QDataStream &ar, bool bIsStoring)
{
	...
	if (bIsStoring)
	{
		...
		{
			CBoat* pBoat=(CBoat*)m_oaBoat.at(ib);
		...
		{
			BoatPolar* pBoatPolar=(BoatPolar*)m_oaBoatPolar.at(ib);
		...
		{
			BoatOpp* pBoatOpp=(BoatOpp*)m_oaBoatOpp.at(ib);
...
void MainFrame::SetGraphSettings(Graph *pGraph)
{
	...
	{
		pSail7->m_BoatGraph[ig].CopySettings(pGraph, false);
	}
}
...
void MainFrame::UpdateBoats()
{
	...
	QSail7 *pSail7 = (QSail7*)m_pSail7;
	CBoat *pBoat, *pCurBoat;

	m_pctrlBoat->clear();

	pCurBoat  = pSail7->m_pCurBoat;

	QStringList BoatNames;
	for (i=0; i<m_oaBoat.size(); i++)
	{
		pBoat = (CBoat*)m_oaBoat[i];
...
void MainFrame::UpdateBoatPolars()
{
	...
	//	else disables the combobox
		...
		CBoat  *pCurBoat = pSail7->m_pCurBoat;
		BoatPolar *pCurBoatPolar  = pSail7->m_pCurBoatPolar;
	...
	//		pSail7->SetBoatPolar();
		...
		{
			pBoatPolar = (BoatPolar*)m_oaBoatPolar[i];
...
void MainFrame::UpdateBoatOpps()
{
	...
	CBoat   *pCurBoat= pSail7->m_pCurBoat;
	BoatPolar *pCurBoatPlr    = pSail7->m_pCurBoatPolar;
	...
	for (int i=0; i<m_oaBoatOpp.size(); i++)
	{
		pBoatOpp = (BoatOpp*)m_oaBoatOpp[i];
	...
	if(pSail7->m_pCurBoatOpp)
	{
		str = QString("%1").arg(pSail7->m_pCurBoatOpp->m_Ctrl,8,'f',3);
