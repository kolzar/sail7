...
//GetElement::funzione che prende in input 3 argomenti formali
/* QDomNode const &node:: il nodo dell'xml
 * QString const &TagName:: nome del nodo???
 * QString const &AttributeName:: nome dell'attributo
*/
QDomElement SailDomDoc::GetElement(QDomNode const &node, QString const &TagName, QString const &AttributeName)
{
    QDomElement en; //un elemento dell'albero DOM
    QDomNode n = node; //un nodo dell'albero DOM

    while(!n.isNull())//loop per trovare tutti i figli
	{
		en = n.toElement(); // try to convert the node to an element.
		if(!en.isNull())
		{
            if(en.tagName()==TagName)//check: nome elemento
			{
                if(en.attribute("name","")==AttributeName) //check: nome attributo
                    return en; //tutto ok allora torna en!!
			}
		}

        // continue searching through child elements... esatto
		QDomNode m = n.firstChild();

        if(!m.isNull())
            en = GetElement(m, TagName, AttributeName);
        else
            en.clear();
...
bool SailDomDoc::Import(QFile *pFile)
{
    ...
    //xml???
	QDomElement en;
    QDomNode n = documentElement().firstChild(); //primo figlio


    en = GetElement(n, QString("string"), QString("sailID"));//get element(node,tag,attribute)
    //controlla se 'en' è vuoto se non lo è allora...
    if(!en.isNull()) pSCSail->m_SailName = en.attribute("value", "No Name");//attribute(name,value)
	else return false;
